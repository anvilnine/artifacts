...
// setup screen refuses to create. Caught by name in server.js, same as AuthFileError.
export class AdminSeedError extends Error {
  constructor(detail) {
    super(
      `ARTIFACTS_ADMIN_USERNAME / ARTIFACTS_ADMIN_PASSWORD rejected: ${detail}. Refusing to ` +
        `start. The username must be 3-32 characters from [a-zA-Z0-9._-] and the password at ` +
        `least 8, which is what the first-run setup screen enforces. Nothing was written, so ` +
        `fix the two variables and boot again. See docs/auth.md#seeding-the-admin-from-env.`,
    );
...
export async function createAuthStore(storage, { apiKey, baseUrl }) {
  ...
  if (!auth.admin && ADMIN_USERNAME && ADMIN_PASSWORD) {
    // The setup screen runs this on the same two fields. Skipping it here seeded accounts the
    // dashboard would have refused, and the operator found out at the login screen.
    try {
      validateCredentials(ADMIN_USERNAME, ADMIN_PASSWORD);
    } catch (err) {
      throw new AdminSeedError(err.message);
    }
