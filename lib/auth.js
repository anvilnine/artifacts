...
// The store owns the auth record and the two secrets. `apiKey` is the bootstrap
// ARTIFACTS_API_KEY; `baseUrl` decides whether cookies get the Secure flag. Matches
// createConfigStore: env is read when the store is built, not at import.
export async function createAuthStore(storage, { apiKey, baseUrl }) {
  ...
  return {
    // The live auth record. Routes read and mutate it in place, then call saveAuth().
    auth,
    SCOPES,
    SESSION_COOKIE,
    DECOY_ADMIN,
