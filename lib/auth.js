...
// decodeURIComponent throw. That reached the error handler as a 500 on every route that reads a
// cookie, including the unauthenticated /a/:slug path, and the browser holding the bad cookie
// kept sending it, so the dashboard stayed broken until the jar was cleared by hand. A value
// that does not decode is not the cookie we are looking for.
...
export async function createAuthStore(storage, { apiKey, baseUrl }) {
  ...
  function sessionPrincipal(req) {
    ...
    // rather than trusted. The old read let a payload with no exp, or a null one, skip the
    // lapse check entirely, which is the shape T1.2.12 fixed on the capability-token path two
    // functions down. Nothing reaches it today, since issueSession stamps exp from a constant
    // and forging a payload needs adminSecret. Making the TTL configurable is what would
    // reopen it.
    //
    ...
    // either. Same spelling as keyExpired.
