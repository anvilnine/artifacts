...
export async function createAuthStore(storage, { apiKey, baseUrl }) {
  ...
  function sessionPrincipal(req) {
    ...
    // Every cookie this server mints carries a numeric exp, so anything else is refused rather
    // than trusted. The old read let a payload with no exp, or a null one, skip the lapse check
    // entirely, which is the shape T1.2.12 fixed on the capability-token path two functions
    // down. Nothing reaches it today, since issueSession stamps exp from a constant and forging
    // a payload needs adminSecret. Making the TTL configurable is what would reopen it.
