...
//
// This catches the typo, not the attacker. It only fires when the target spells the server the
// way BASE_URL spells it, and a deploy that never set BASE_URL is running on the default
// http://localhost:PORT, so behind a proxy no real target ever matches and the check is off.
// Any other name or IP that reaches the same server walks past it too, as does a pair of
// artifacts pointing at each other and a target on another host that redirects back here.
// Someone who wants a loop can still have one. Someone who pasted the wrong URL gets told.
//
...
// same loop. One trailing dot comes off each host name first: DNS treats "example.com." as the
// same name, and so does the server, so a target carrying the dot reaches the same routes.
export function pointsAtOwnSlug(target, slug, baseUrl) {
  ...
  if (bareHostname(url) !== bareHostname(self) || url.port !== self.port) return false;
  ...
  // express matches the route against the raw path and decodes only the segment it captured,
  // so the split happens before the decode here too. /a%2fhop is one segment that decodes to
  // "a/hop" and matches no route (a 404), while /a/%68op is two and reaches the artifact
  // named hop. Decoding the whole path would call the first one a loop and refuse a target
  // that goes nowhere.
  //
  // Both comparisons fold case, because express routes case-insensitively unless `case
  // sensitive routing` is set: /A/HOP reaches the artifact named hop and loops the same way.
  const segments = path.split('/');
  if (segments.length !== 3 || segments[0] !== '' || segments[1].toLowerCase() !== 'a') return false;
  let name;
  try {
    name = decodeURIComponent(segments[2]);
  } catch {
    return false; // a segment that will not decode reaches no artifact
  }
  return name.toLowerCase() === slug.toLowerCase();
}

// One trailing dot off a host name. "example.com." is the same name to DNS and reaches the
// same server, so a target that carries it is the same host as a BASE_URL that does not.
function bareHostname(url) {
  return url.hostname.endsWith('.') ? url.hostname.slice(0, -1) : url.hostname;
}

// The same check for a caller holding a stored record instead of a target string: a rename is
// the other way a redirect ends up pointing at itself, and the record it checks may predate
// meta.target, which is what resolveRedirectTarget exists for. Reading meta.target straight off
// the record makes new URL(undefined), which throws inside pointsAtOwnSlug and quietly answers
// false, so the rename closes the loop. Returns the target it refused, or null when the rename
// is fine, so the caller can name the target in its 400.
export async function storedTargetPointsAtSlug({ meta, readSource, slug, baseUrl }) {
  const target = await resolveRedirectTarget({ meta, readSource });
  return target && pointsAtOwnSlug(target, slug, baseUrl) ? target : null;
}
...
//
// Every stored redirect counts, disabled and expired ones included. A record that is not
// serving today is one PATCH away from serving again, and a leaked key that minted fifty hops
// is the same problem whether or not the operator has switched them off yet.
//
