...
// that sets none. og:image has to be absolute, so a root-relative logo is resolved against the
// base URL the server builds its links from.
export function socialBranding(branding, baseUrl) {
  return {
    siteName: branding.productName,
    image: branding.logoUrl.startsWith('/') ? `${baseUrl}${branding.logoUrl}` : branding.logoUrl,
  };
}
