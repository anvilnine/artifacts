...
function labelOn(rgb) {
  const fill = relativeLuminance(rgb);
  return contrast(fill, DARK_LABEL_LUMINANCE) >= contrast(fill, 1)
    ? BTN_LABEL_DARK
    : BTN_LABEL_LIGHT;
}
...
// color-mix(in srgb, X n%, white) in the same coordinates CSS uses, so the label picked here is
// picked for the color the browser will actually paint.
function mixedWithWhite(rgb, percent) {
  const mix = (c) => (c * percent + 255 * (100 - percent)) / 100;
  return { r: mix(rgb.r), g: mix(rgb.g), b: mix(rgb.b) };
}

// The label for a hover fill. Both buttons hover to a lighter mix of the accent, and the label
// picked for the resting fill can fail on it: #0055ff picks white, and white on the console's
// hover mix reads 3.35:1 where the same white on the resting fill reads 5.61:1.
function hoverLabel(branding, percent) {
  if (!branding.accentColor) return BTN_LABEL_DARK;
  const rgb = accentRgb(branding.accentColor);
  if (!rgb) return BTN_LABEL_DARK;
  return labelOn(mixedWithWhite(rgb, percent));
}
...
export function frameBrandSlots(branding) {
  // alt is the product name when there is one. The wordmark is suppressed whenever a logo is
  // set, so an empty alt left a screen reader with no publisher identity in the bar at all. No
  // product name and there is nothing to say, so it stays decorative.
  const logo = branding.logoUrl
    ? `<img src="${escapeHtml(branding.logoUrl)}" alt="${escapeHtml(branding.productName)}">`
    : '';
  ...
  // The wordmark went at 560px and the logo did not, so a 120px logo on a 390px screen left the
  // title 79px for 140px of text. Same width, smaller mark, rather than no mark at all: a logo
  // is the only brand a framed page has once the wordmark is gone.
  if (logo) rules.push('  @media (max-width: 560px) { #brand img { max-width: 56px; } }');
  return {
...
export function passwordBrandSlots(branding) {
  ...
  // The button keeps BTN_FG at rest and swaps to the label the hover mix needs. It rides in the
  // appended block rather than the shell so an unbranded page keeps the bytes it shipped with.
  if (branding.accentColor) rules.push(`  button:hover { color: ${hoverLabel(branding, 80)}; }`);
  return {
    FAVICON: faviconTag(branding),
    LOGO: logo,
    FOOTER: footer,
...
export function dashboardBrandSlots(branding) {
  ...
  if (branding.accentColor) {
    ...
    // button:hover swaps the fill to --rose, which is the accent 70% of the way to white, so the
    // label the resting fill picked can fall under the line on hover.
    rules.push(`  button:hover { color: ${hoverLabel(branding, 70)}; }`);
