...
export function pdfViewerFlags(settings) {
  ...
  // minimal drops our own toolbar, so with downloads on the browser's is the only way left to
  // reach the file. Asking for that one to go too left "PDF download: on" with no button
  // anywhere on the page, which is the label and the page disagreeing.
  if (mode === 'presentation' || !download) params.push('toolbar=0', 'navpanes=0');
  return {
    mode,
    download,
    bar: mode !== 'minimal',
