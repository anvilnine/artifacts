// Server config: global frame settings, markdown render defaults and the branding the
// viewer-facing shells read, stored through the storage backend. Routing config through
// storage keeps the global frame setting as durable as artifacts: it survives a
// fresh-container restart on the s3/git/postgres backends too.

import { ApiError } from './errors.js';
import { defaultBranding, normalizeBranding, updateBranding } from './branding.js';
...
function normalizeConfig(raw) {
  ...
  return {
    ...
    branding: normalizeBranding(raw?.branding, defaults.branding),
  };
}
...
export async function createConfigStore(storage) {
  ...
  return {
    ...
    async update(patch) {
      ...
      const branding = updateBranding(patch?.branding, current.branding);
      current = {
        frame: {
