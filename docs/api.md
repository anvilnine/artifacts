...
- Body limits by caller: a key with `publish` scope or better, and an admin session, get 10 MB of JSON (50 MB on the zip route). Everyone else gets 256 kB, because no route here takes a bigger body below `publish` scope. The credential routes (`/api/auth/*`, `/a/:slug/unlock`) get 16 kB whoever is calling.
- Over whichever body limit applied is a `413` with `{"error":"body too large: the limit on this request is 256 kb"}`, naming the limit the request actually hit rather than every limit there is. A body the JSON parser refuses is a `400` with `{"error":"invalid JSON body: expected a JSON object"}`, which includes a bare `null`, `"x"`, `5` or `true`: the parser is in strict mode, so only an object or an array counts as a body. A body the parser refuses before it gets that far answers its own `4xx` carrying the parser's own message, and the full set is `invalid JSON body: expected a JSON object` (400), `unsupported charset "X"` (415), `unsupported content encoding "X"` (415), `content encoding unsupported` (415), `request aborted` (400) and `request size did not match content length` (400). Each one repeats only what the request itself carried. Everything else is a `500` with `{"error":"internal server error"}` and nothing else, apart from the one `503` in the write-queue bullet below, so a 5xx still never means your request was malformed.
- Large bodies from a caller below `publish` scope are capped: 20 a minute per client IP, counting any request that carries a body and declares 256 kB or more in `Content-Length`, whatever its method or route. A chunked body declares no length and never counts; the 256 kB parser below is what bounds it. Over the budget is a `429` with `Retry-After` and `{"error":"too many large publishes, try again later"}`. The check runs above the body parser, so a refused request is never buffered. A key that may publish and an admin session never count against it, and an ordinary write is a few kB and never touches it. See [rate limiting and the edge](deploy.md#rate-limiting-and-the-edge).
- Writes to one slug run one at a time, so a `PUT`, `PATCH`, `DELETE` or duplicate waits for the write ahead of it. One write gets 3 minutes, which clears the longest publish this server can be asked to make (a zip deploy is one handler writing every file in the archive). A storage call that has not answered by then answers `503` with `{"error":"the storage backend did not answer in time; retry"}` and `Retry-After: 5`. The server is fine and that one call is not, which is why it is worth retrying. The slug is not handed on: the stalled call is still running and cannot be cancelled, so letting the next write land underneath it meant the stalled one put its own snapshot back on top and undid a visibility flip or a token rotation with a `200` on it. It may still land later, so a retry can find the change already made.
- Everyone queued behind that write hears the same `503` at the same moment, and their writes are dropped where they stand rather than run later. Otherwise the caller in front got its answer and the callers in line got nothing at all: their writes never started, so their clocks never ran, and nothing else here closes an idle socket. A failure that is not a storage timeout (a `404` on a missing slug, a `409` on a taken one) belongs to the caller that made it and does not travel down the queue.
- `type` is required on a `PUT` of anything that is not html. A `PUT` replaces the artifact, and `type` falls back to `html` when the body leaves it out, so an update that omits it rewrites a jsx, tsx, md or redirect artifact as an html page and deletes the files the old type owned, the source you uploaded among them. Every other optional field goes the other way and keeps its stored value when omitted (`title`, `tags`, `project`, `description`, `ogImage`, `frame`, `visibility`, `pdf`), which is what makes this one easy to miss. Two types refuse the call instead of converting: a pdf answers `400` (below), and a zip site refuses an inline `PUT` at all. `PATCH` never touches the body or the type, so reach for it when you only want to change metadata.
...
- `PUT` on a pdf must name `type`. Where any other artifact converts, a pdf would lose bytes the caller cannot rebuild from anything it holds, so it is a `400` instead. Pass `type: "pdf"` to send new bytes, or another type to convert the artifact and drop the file.
...
- Link previews: `description` and `ogImage` are what a chat app shows when someone pastes the link. `description` is one line, max 300 characters, with runs of whitespace collapsed to single spaces. `ogImage` must be an absolute `http://` or `https://` URL and cannot carry a username or password, capped at 2048 characters on the normalized URL; another artifact's URL works, a relative path does not, because the chat app fetches the image from its own base. Anything else is a `400`, not a silent drop. `PUT` keeps both when they are omitted, the way it keeps `tags` and `project`; `PATCH` with `""` clears one. Both ride `GET /api/artifacts`. The tags render in the viewer frame, in a markdown page and in the pdf viewer page, so an html, jsx, tsx or zip artifact carries them only while it is framed, and a redirect stores them and renders nothing. Both keep storing and both keep listing either way, so the dashboard dialogs and the CLI say when a preview will not show rather than letting it look set. Full rules in [Link previews](formats.md#link-previews).
...
- What those two statuses look like depends on who asked. A request whose `Accept` header names `text/html`, which is what a browser sends for a navigation, gets the branded card built from `shells/not-found.html`: `404 Artifact unavailable`, or `410 Artifact expired` with copy that says the expiry date passed. Anything else (curl, `fetch()`, an `<img>`, a range read) keeps the one-line `not found` / `artifact expired` body, so an asset request never gets an HTML page in place of the asset. `GET /a/:slug` itself always answers with the card, the way it always has for a miss. The JSON API routes are unaffected and keep their `{error}` bodies.
