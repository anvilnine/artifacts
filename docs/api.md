...
- Body limits: 10 MB JSON, 50 MB zip. Over the limit is a `413` with `{"error":"body too large (10mb json / 50mb zip / 16kb on credential routes)"}`. A body the JSON parser refuses is a `400` with `{"error":"invalid JSON body: expected a JSON object"}`, which includes a bare `null`, `"x"`, `5` or `true`: the parser is in strict mode, so only an object or an array counts as a body. A body the parser refuses before it gets that far answers its own `4xx` carrying the parser's own message, and the full set is `invalid JSON body: expected a JSON object` (400), `unsupported charset "X"` (415), `unsupported content encoding "X"` (415), `content encoding unsupported` (415), `request aborted` (400) and `request size did not match content length` (400). Each one repeats only what the request itself carried. Everything else is a `500` with `{"error":"internal server error"}` and nothing else, apart from the one `503` in the next bullet, so a 5xx still never means your request was malformed.
- The write routes (`POST`, `PUT`, `PATCH` under `/api/artifacts`) cap how many large bodies one client IP may send: 20 a minute, counting only requests that declare 256 kB or more. Over that is a `429` with `Retry-After` and `{"error":"too many large publishes, try again later"}`. The check runs above the body parser, so a refused request is never buffered. Ordinary writes are a few kB and never count against it. See [rate limiting and the edge](deploy.md#rate-limiting-and-the-edge).
- Writes to one slug run one at a time, so a `PUT`, `PATCH`, `DELETE` or duplicate waits for the write ahead of it. One write gets 30 seconds. A storage call that has not answered by then gives the slug back to the next writer and answers `503` with `{"error":"the storage backend did not answer in time; retry"}` and `Retry-After: 5`. The server is fine and that one call is not, which is why it is worth retrying. The stalled write is not cancelled, so it may still land: a retry can find the change already made. Before this, a storage call that never answered parked every later write to that slug for the life of the process while reads kept working, so the artifact looked healthy and could not be managed.
